...
#define SAVELOCATION (EEPROM_STORAGE_SPACE_START + 591)
...
void gameOver()
{
  if (score > highScore) EEPROM.put(SAVELOCATION, highScore); delay(100);
  arduboy.clear();
  arduboy.setTextSize(1);
  arduboy.setCursor(25, 30);
  arduboy.print("HIGH SCORE " + String(highScore));
  arduboy.setTextSize(1);
...
void mainGame()
{
  ...
  /* do
    {
     if ((millis() - initialDelay) > 500) waitState = 0;
     arduboy.display();
    }   while (waitState == 1);
  */
  checkButtons();
